...
{
    public class TransformFindCodeGeneratorRuleHelper : ICodeGeneratorRuleHelper
    {
        ...
        public string GetGeneratorCode(List<ReferenceBindComponent.BindObjectData> bindDataList, string nameSpace, string className,object userData)
        {
            ...
            stringBuilder.AppendLine($"{indentation}\tprivate Transform m_Transform;");
            stringBuilder.AppendLine($"{indentation}\tpublic {className}(Transform transform)");
            stringBuilder.AppendLine($"{indentation}\t{{");
            stringBuilder.AppendLine($"{indentation}\t\tm_Transform = transform;");
            stringBuilder.AppendLine($"{indentation}\t}}");
            //组件字段
            ...
            Transform transform = (Transform)userData;
            //属性字段
            foreach (ReferenceBindComponent.BindObjectData data in bindDataList)
            {
                stringBuilder.AppendLine($"{indentation}\tpublic {data.BindObject.GetType().Name} {data.FieldName}");
                stringBuilder.AppendLine($"{indentation}\t{{");
                stringBuilder.AppendLine($"{indentation}\t\tget");
                stringBuilder.AppendLine($"{indentation}\t\t{{");
                stringBuilder.AppendLine($"{indentation}\t\t\tif(m_{data.FieldName} == null)");
                stringBuilder.AppendLine($"{indentation}\t\t\t{{");
                string path = GetPath(transform, ((Component)data.BindObject).transform);
                if (string.IsNullOrEmpty(path))
                {
                    stringBuilder.AppendLine($"{indentation}\t\t\t\tm_{data.FieldName} = m_Transform.GetComponent<{data.BindObject.GetType().Name}>();");
                }
                else
                {
                    stringBuilder.AppendLine($"{indentation}\t\t\t\tm_{data.FieldName} = m_Transform.Find(\"{path}\").GetComponent<{data.BindObject.GetType().Name}>();");
                }

                stringBuilder.AppendLine($"{indentation}\t\t\t}}");
                stringBuilder.AppendLine($"{indentation}\t\t\treturn m_{data.FieldName};");
                stringBuilder.AppendLine($"{indentation}\t\t}}");
                stringBuilder.AppendLine($"{indentation}\t}}");
            }
            stringBuilder.AppendLine($"{indentation}}}");
        ...
        public bool GeneratorCodeAndWriteToFile(List<ReferenceBindComponent.BindObjectData> bindDataList,
            string nameSpace, string className, string folderPath,object userData)
        {
